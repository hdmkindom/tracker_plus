...
RecordNode::RecordNode(const rclcpp::NodeOptions & options) : Node("rm_auto_record", options)
{
    ...
    topic_name = this->declare_parameter<std::vector<std::string>>("topic_name", std::vector<std::string>());

    topic_type = this->declare_parameter<std::vector<std::string>>("topic_type", std::vector<std::string>());
