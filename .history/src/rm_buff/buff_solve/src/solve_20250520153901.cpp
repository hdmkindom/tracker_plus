 ...
 */
std::pair<float, float> Solve::calculatePitchAndYaw(int idx, const buff_interfaces::msg::Blade::SharedPtr& msg, float timeDelay, float s_bias, float z_bias, float current_v, bool use_target_center_for_yaw,float& aim_x, float& aim_y, float& aim_z) {
    ...
    // 切换识别装甲板还是robt 中心
    float yaw_x = use_target_center_for_yaw ? msg->pose.position.x : aim_x;
    float yaw_y = use_target_center_for_yaw ? msg->pose.position.y : aim_y;
