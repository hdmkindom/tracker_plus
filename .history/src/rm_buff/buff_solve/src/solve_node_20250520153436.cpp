    ...
    const buff_interfaces::msg::BladeArray::SharedPtr Blade_Array_msg)
{
    ...
    // Init message
    blade_msg = Blade_Array_msg->blades[0];
    tar_yaw = blade_msg->yaw;
    int idx = 0;
    bool is_fire = false;
    ...
    tar_yaw += blade_msg->v_yaw * time_delay;
    ...
    //   3     1
    //      0
    if (blade_msg->buffs_num == ARMOR_NUM_OUTPOST) {
        solve_ -> calculateArmorPosition(blade_msg, false, true, tar_yaw);
    }
    // 对于普通装甲板 
    else {
        //* 解算装甲板位置 注意 use_1 以及 use_average_radius
        solve_ -> calculateArmorPosition(blade_msg, false, false, tar_yaw);
    }
        // 找到四块装甲板中最适合开火的那一块 idx ,并且如果都不适合就以最后一块也就是当前追踪的装甲板为准
        idx = solve_ -> selectArmor(blade_msg);
