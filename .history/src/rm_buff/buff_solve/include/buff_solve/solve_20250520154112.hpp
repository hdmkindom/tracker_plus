...
public:
    ...
    std::pair<float, float> calculatePitchAndYaw(
        int idx, const buff_interfaces::msg::Blade::SharedPtr& msg, float timeDelay, float s_bias, float z_bias, float current_v, bool use_target_center_for_yaw,float& aim_x, float& aim_y, float& aim_z);
