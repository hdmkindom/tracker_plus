  ...
  const sensor_msgs::msg::Image::ConstSharedPtr & image_msg)
{
  ...
  // Draw latency
  auto latency = (end - start).seconds() * 1000;
  std::stringstream latency_ss;
  latency_ss << "latency_msg: " << std::fixed << std::setprecision(2) << latency_msg.data << "ms";
  auto latency_s = latency_ss.str();

  cv::putText(
  img_copy, latency_s, cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(255, 0, 0), 2);
