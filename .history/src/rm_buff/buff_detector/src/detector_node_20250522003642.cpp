...
void BuffDetectorNode::imageCallback(const sensor_msgs::msg::Image::ConstSharedPtr msg)
{
  ...
  if (pnp_solver_ != nullptr && is_buff_task_) 
  {
    ...
    buff_interfaces::msg::DebugBladeArray debug_blade_array;
        ...
        blade_msg.label = 1;
        blade_msg.prob = 1.0;
        blade_msg.pose.position.x = 2;
        blade_msg.pose.position.y = 2;
        blade_msg.pose.position.z = 2;

        blade_msg.pose.orientation.w = 1;
        blade_msg.pose.orientation.x = 1;
        blade_msg.pose.orientation.y = 1;
        blade_msg.pose.orientation.z = 1;
  ...
  const sensor_msgs::msg::Image::ConstSharedPtr & image_msg)
{
  ...
  // Draw latency
  cv::putText(
  img_copy, latency_msg.data, cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(255, 0, 0), 2);
