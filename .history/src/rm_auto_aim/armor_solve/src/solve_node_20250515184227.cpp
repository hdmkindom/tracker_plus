...
: Node("armor_solve", options)
{
    ...
    // 用于弹道解算的k值具体计算在git
    float k = this->declare_parameter("solve.k", 0.092);
    ...
    // 
    // 解算的智能指针
    solver_ = std::make_unique<Solve>(k, bias_time, s_bias, z_bias, fire_k);
...
void ArmorSolveNode::targetCallback(const auto_aim_interfaces::msg::Target::SharedPtr target_msg)
{
    ...
    float timeDelay = delay_time/1000.0 + solve_->ftime;  // 改为 测距 所用时间 

    tar_yaw += target_msg->v_yaw * timeDelay;
    ...
    pitch_and_yaw = solve_ -> calculatePitchAndYaw(idx, target_msg, timeDelay, s_bias, z_bias, current_v, false, aim_x, aim_y, aim_z);
