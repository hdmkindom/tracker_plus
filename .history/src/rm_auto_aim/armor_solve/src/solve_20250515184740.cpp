 ...
 */
void Solve::calculateArmorPosition(int idx, const auto_aim_interfaces::msg::Target::SharedPtr msg, bool use_1, bool use_average_radius, float tar_yaw) {

    std::vector<float> tmp_yaws; 

    auto min_yaw_in_cycle = std::numeric_limits<float>::max();   //?何用之有
    auto max_yaw_in_cycle = std::numeric_limits<float>::min();
    // 对每块装甲板
