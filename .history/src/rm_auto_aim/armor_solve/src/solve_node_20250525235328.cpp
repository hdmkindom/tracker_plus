    ...
    const auto_aim_interfaces::msg::Target::SharedPtr target_msg)
{
    ...
    // 检查 target_msg 是否有效
    if (!target_msg || target_msg->armors_num == 0) {
        RCLCPP_WARN(this->get_logger(), "No target detected, resetting aim position to (0, 0, 0)");

        // 发布默认消息
        auto_aim_interfaces::msg::Send send_msg;
        auto_aim_interfaces::msg::SolveInfo solve_info_msg;

        send_msg.header.stamp = this->get_clock()->now();
        send_msg.id = "";
        send_msg.pitch = 0.0;
        send_msg.yaw = 0.0;
        send_msg.is_fire = false;

        solve_info_msg.header.stamp = this->get_clock()->now();
        solve_info_msg.tar_yaw = 0.0;
        solve_info_msg.idx = -1;
        solve_info_msg.aim_position.x = 0.0;
        solve_info_msg.aim_position.y = 0.0;
        solve_info_msg.aim_position.z = 0.0;
        solve_info_msg.d_yaw = 0.0;
