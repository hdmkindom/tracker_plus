  ...
  serial_driver_{new drivers::serial_driver::SerialDriver(*owned_ctx_)}
{
  ...
  record_switch_ = this->declare_parameter("record_switch", 0);
  ...
  receive_pub_ = this->create_publisher<auto_aim_interfaces::msg::Receive>("/receive", rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(rmw_qos_profile_sensor_data)));
    ...
    try {
      ...
      if (header[0] == 0x5A) {
        ...
        if (crc_ok) {
          ...
          if (record_switch_ == 1) {
            record_controller.data =  "start";  // 开始录制
          } else {
            record_controller.data = "stop";  // 关闭录制
          }
...
void RMSerialDriver::reopenPort()
{
  ...
  try {
    ...
    if (rclcpp::ok()) {
      ...
      RCLCPP_ERROR(get_logger(), "串口尝试自重启");
      reopenPort();
    }
  }
}
